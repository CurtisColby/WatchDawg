...
PIN Lock:
- GET /library returns an empty list with locked_hidden=True when PIN lock
  is configured and no valid session token is supplied.
- Stream and thumb endpoints are NOT gated — direct URL access to known paths
  is acceptable since the paths are not guessable without first seeing the list.
"""
...
async def list_library(
    ...
    PIN lock: Returns empty list with locked_hidden=True when locked.
    """
    ...
    if not unlocked:
        return {
            "total": 0,
            "directory": settings.music_videos_path,
            "files": [],
            "locked_hidden": True,
        }
    ...
    if not os.path.isdir(music_dir):
        return {"total": 0, "files": [], "directory": music_dir, "locked_hidden": False}
